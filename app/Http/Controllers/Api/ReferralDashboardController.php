...
use App\Services\Referral\ReferralDashboardService;
...
class ReferralDashboardController extends Controller
{
    public function __construct(
        protected ReferralDashboardService $referralDashboardService
    ) {}

    /**
     * Facility referral intelligence dashboard (single payload).
     *
    ...
    public function show(Request $request, int $facilityId): JsonResponse
    {
        try {
            ...
            $result = $this->referralDashboardService->getDashboard(
                $facilityId,
                $request->query('tz')
            );
            ...
            Log::error('Referral dashboard HTTP failed', [
            ...
            return response()->json([
                'success' => false,
                'message' => 'Unable to load referral dashboard.',
            ], 500);
        }
    }
}
