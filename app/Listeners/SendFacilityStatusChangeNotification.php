...
class SendFacilityStatusChangeNotification implements ShouldHandleEventsAfterCommit
{
    ...
    public function handle(FacilityStatusChanged $event): void
    {
        ...
        // Get facility owners to notify
        $owners = $this->getFacilityOwners($facility->id);

        if ($owners->isEmpty()) {
            Log::info('No owners found for facility, skipping notification', [
        ...
        // Send personalized notification to each owner
        foreach ($owners as $owner) {
            ...
            $tempUser = new User();
            $tempUser->id = $owner['user_id'];
            $tempUser->first_name = $owner['first_name'];
            $tempUser->last_name = $owner['last_name'];
            $tempUser->display_name = $owner['display_name'];
            $tempUser->email_encrypted = encrypt($owner['email']);

            $this->notificationService->sendToUser(
                user: $tempUser,
                title: $this->getTitle($newStatus, $owner),
    ...
    private function getFacilityOwners(int $facilityId): \Illuminate\Support\Collection
    {
        return DB::table('facility_owners')
            ->join('staff', 'facility_owners.staff_id', '=', 'staff.id')
            ->join('users', 'staff.user_id', '=', 'users.id')
            ->where('facility_owners.facility_id', $facilityId)
            ->select(
                'users.id as user_id',
                'users.first_name',
                'users.last_name',
            ...
            ->map(function ($owner) {
                ...
                try {
                    ...
                    Log::warning('Failed to decrypt owner email', [
    ...
    private function getTitle(string $newStatus, array $owner): string
    {
        $firstName = $this->getFirstName($owner);
        
        return match ($newStatus) {
            'suspended' => "Important: {$firstName}, Your Facility Has Been Temporarily Suspended",
            'banned' => "Urgent: {$firstName}, Your Facility Has Been Permanently Banned",
            'active' => "Good News: {$firstName}, Your Facility Has Been Reactivated",
            default => "{$firstName}, Important Update Regarding Your Facility",
        };
    }

    private function getFirstName(array $owner): string
    {
        if (!empty($owner['display_name'])) {
            return explode(' ', $owner['display_name'])[0];
        }
    ...
    private function buildPersonalizedEmailBody(
        ...
        $supportEmail = 'custocare@custospark.com';
        $supportPhone = '+256 756 697 871';

        // Personalized opening based on status change
        $openings = [
            'suspended' => "I hope this message finds you well. After a thorough review, we've had to take action regarding <strong>{$facilityName}</strong>.",
            'banned' => "I'm writing to you today with difficult news regarding <strong>{$facilityName}</strong>.",
            'active' => "Great news! I'm pleased to inform you that we've reinstated access to <strong>{$facilityName}</strong>.",
        ];

        $opening = $openings[$newStatus] ?? "We're writing to update you about <strong>{$facilityName}</strong>.";

        $statusMessages = [
            'suspended' => [
                'title' => 'Temporary Suspension',
                'description' => "After careful consideration, your facility '{$facilityName}' has been temporarily suspended from {$appName}.",
                'action' => 'We genuinely want to work with you to resolve this matter. Please review the reason below and reach out to our compliance team so we can help you restore full access.',
                'urgency' => 'This is a temporary measure while we work together to address the concerns.',
            ],
            'banned' => [
                'title' => 'Permanent Ban',
                'description' => "After multiple reviews and careful consideration, we've made the difficult decision to permanently ban '{$facilityName}' from {$appName}.",
                'action' => 'We understand this is disappointing news. If you believe this decision was made in error, you may appeal by contacting our compliance team within 14 days.',
                'urgency' => 'This action is permanent unless a successful appeal is made.',
            ],
            'active' => [
                'title' => 'Facility Reactivated',
                'description' => "We're happy to inform you that full access to '{$facilityName}' has been restored on {$appName}.",
                'action' => 'Your staff can now log in and resume normal operations. We appreciate your cooperation in resolving the previous issues.',
                'urgency' => 'No immediate action is required, but we encourage you to review our  guidelines to prevent future disruptions.',
            ],
        ];

        $statusInfo = $statusMessages[$newStatus] ?? [
            'title' => 'Status Updated',
            'description' => "The status of '{$facilityName}' has been changed from '{$oldStatus}' to '{$newStatus}'.",
        ...
        <div style='margin-top: 32px; padding-top: 24px; border-top: 1px solid #e5e7eb;'>
            <p style='margin: 0 0 8px 0; font-size: 14px; color: #6b7280;'>
                Thank you for your understanding and partnership, {$firstName}.
            </p>
